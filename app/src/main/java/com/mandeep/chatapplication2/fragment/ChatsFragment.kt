...
import com.mandeep.chatapplication2.Connectivity.State
...
import com.mandeep.chatapplication2.MainActivity
...
class ChatsFragment : Fragment() /*,BottomSheetDialogue3.WallpaperListener_fragment*/,WallpaperAdapter.WallpaperListener{
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.sendButton.setOnClickListener {
            ...
            firebaseFirestore.collection(Constants.KEY_COLLECTION_CHAT).add(map).addOnSuccessListener {
                ...
                receiverUser_cloud_token?.let {
                    ...
                    FCMHttpServerRequest.sendNotification(it,senderName.toString(),message.toString())
                }
                        ...
                        firebaseFirestore.collection(Constants.KEY_COLLECTION_CHAT).document(it.id.toString()).update(Constants.KEY_DOC_ID,it.id.toString())
                ...
                chatList.forEach { chat ->
                    ...
                    if(senderId.equals(chat.receiverId))
                    {
                        firebaseFirestore.collection(Constants.KEY_COLLECTION_CHAT).document(chat.doc_id.toString()).update(Constants.KEY_IS_SEEN,true).addOnSuccessListener {
                            Log.d("dvmdgd",chat.doc_id.toString()+"       "+chat.message.toString())
                        }
                    }
                }
